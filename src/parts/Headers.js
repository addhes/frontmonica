...
export default function Header({ onLight }) {
  ...
  useEffect(() => {
    const userCookies =
      decodeURIComponent(window.document.cookie)
        ?.split(";")
        ?.find?.((item) => item.indexOf("MONICA:user") > -1)
        ?.split("=")[1] ?? null;
  ...
  return (
    ...
    >
      ...
      <ul
        className={[
          "transition-all duration-200 items-center fixed inset-0 bg-indigo-1000 pt-24 md:pt-0 md:bg-transparent md:relative md:flex md:opacity-100 md:visible md:justify-end",
        ...
        <li className="mt-8 md:mt-0">
          {User ? (
            <a
              ...
              href={linkCTA}
              className="inline-flex hover:bg-indigo-800 transition-all duration-200 text-white hover:text-teal-500 text-lg px-6 py-3 font-medium ml-6"
            >
              <span className="rounded-full overflow-hidden mr-3 border-2 border-orange-500 items-center">
                {User?.thumbnail ? (
                  <img
                    src={User?.thumbnail}
